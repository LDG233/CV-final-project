...
class SAN_decoder(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.counting_channel = params['counting_decoder']['out_channel']
        self.word_num = params['word_num']
        self.dropout_prob = params['dropout']
        self.device = params['device']
        self.word_num = params['word_num']
        self.struct_num = params['struct_num']
        self.struct_dict = [115, 116, 117, 118, 119, 120, 121]
        ...
        self.counting_state_weight = nn.Linear(self.counting_channel, self.hidden_size // 2)
    ...
    def forward(self, cnn_features, counting_preds, labels, images_mask, labels_mask, is_train=True):
        ...
        counting_weight = self.counting_state_weight(counting_preds)
        ...
        if is_train:
            ...
            for i in range(num_steps):
                ...
                if self.params['dropout']:
                    word_out_state = self.dropout(current_state + word_weighted_embedding + word_context_weighted + counting_weight)
                    c2p_out_state = self.dropout(c2p_state + c2p_weighted_word + c2p_weighted_relation + c2p_context_weighted + counting_weight)
                else:
                    word_out_state = current_state + word_weighted_embedding + word_context_weighted + counting_weight
                    c2p_out_state = self.dropout(c2p_state + c2p_weighted_word + c2p_weighted_relation + c2p_context_weighted + counting_weight)
